...
namespace System
{
    ...
    /// 公用扩展类
    /// </summary>
    public static class NetWorkGroupExtensions
    {
        #region [基础信息相关]
        /// <summary>
        /// 执行转换，常应用在枚举转换
        ...
        public static IEnumerable<SelectListItem> ShowSelectListItemByTree<T, TType>(this IQueryable<T> Data) where T : IModelDropDown<TType>
        {
            ...
            if (Data.Where(p => p.PID.Equals(0) || p.PID == null).Count() > 0)
            {
            ...
            else if (Data.Where(p => !p.PID.Equals(0) && Data.Count(x => x.PID.Equals(p.ID)) > 0).Count() > 0)
            {
                foreach (var item in Data.Where(p => !p.PID.Equals(0) && Data.Count(x => x.PID.Equals(p.ID)) > 0))
                {
                    list.Add(new SelectListItem() { Text = item.Name.Trim(), Value = item.ID.ToString() });
                    ShowInner<T, TType>(Data, item.ID, list);
                }
        ...
        /// <summary>
        /// 根据一个父菜单生成子菜单的DropDownList选择项
        /// </summary>
        /// <typeparam name="T">实现了IModelDropDown&lt;T&gt;的类型</typeparam>
        /// <typeparam name="TType">表示主接口IModelID&lt;T&gt;的名称为Name的类型</typeparam>
        /// <param name="Data">IQueryable&lt;T&gt;数据</param>
        /// <returns>IEnumerable&lt;SelectListItem&gt;</returns>
        public static IEnumerable<SelectListItem> ShowSelectListItemByTree<T, TType>(this IQueryable<T> Data, T thisItem) where T : IModelDropDown<TType>
        {
            List<SelectListItem> list = new List<SelectListItem>();
            list.Add(new SelectListItem() { Text = "【请选择】", Value = "0", Selected = true });
            if (Data.Where(p => p.PID.Equals(thisItem.ID)).Count() > 0)
            {
                foreach (var item in Data.Where(p => p.PID.Equals(0) || p.PID == null))
                {
                    list.Add(new SelectListItem() { Text = item.Name.Trim(), Value = item.ID.ToString() });
        ...
        /// <summary>
        /// 用于生成一个下拉列表的可枚举对象，此类行为可枚举的SelectListItem
        /// <para>TModel不需要实现IModelDropDown,不用于树形下拉列表，但需要实现IModelID,IModelName</para>
        /// </summary>
        /// <typeparam name="TModel">实现IModelID,IModelName的对象</typeparam>
        /// <param name="data">数据数组</param>
        /// <param name="IsCreateEmptyRow">是否创建一个包含【请选择】值为空的列</param>
        /// <returns>下拉列表可用数组</returns>
        public static IEnumerable<SelectListItem> CreateSingleDropDownList<TModel>(this IQueryable<TModel> data, Boolean IsCreateEmptyRow = true) where TModel : IModelID<Int64>, IModelName
        {
            List<SelectListItem> _list = new List<SelectListItem>();
            if (IsCreateEmptyRow)
            {
                _list.Add(new SelectListItem() { Text = "【请选择】", Value = "", Selected = true });
            }
            foreach (var item in data)
            {
                _list.Add(new SelectListItem() { Text = item.Name.Trim(), Value = item.ID.ToStringTrim() });
            }
            return _list;
        }
        #endregion
        ...
        /// <param name="name">CheckBoxList名称</param>
        /// <param name="items">用于生成列表的类型</param>
        /// <param name="CheckedID">在使用Update方法时将已得到的值选中</param>
        /// <returns>CheckBoxList</returns>
        public static MvcHtmlString CheckBoxList(this HtmlHelper helper, string name, IEnumerable<SelectListItem> items, String CheckedID = "")
        {
            var str = new StringBuilder();
            str.Append(@"<div class=""checkboxlist""><ul>");

            foreach (var item in items)
            {
                str.Append("<li><input type=\"checkbox\" id=\"" + name + "-" + item.Value + "\" name=\"" + name + "\" value=\"" + item.Value + "\"");
                if (!String.IsNullOrEmpty(CheckedID))
                {
                    foreach (var checkeds in CheckedID.Split(','))
                    {
                        if (item.Value == checkeds) { str.Append(@" checked=""chekced"""); }
                    }
                }
                if (item.Selected)
                ...
                str.Append(" />");
                str.Append("<label for=\"" + name + "-" + item.Value + "\">" + item.Text + "</label>");
                str.Append("</li>");
            }
        ...
        /// <param name="LastString">末尾的字符串</param>
        /// <returns>返回切除后的结果</returns>
        public static String CutLastString(this String str, String LastString)
        {
            String ret = String.Empty;
            if (str.EndsWith(LastString))
            {
                ret = str.Substring(0, str.Length - LastString.Length);
            }
            else
            {
                ret = str;
                        ...
                        using (System.Drawing.Image oldImage = new Bitmap(fileinfo.FullName.Trim()))
                        {
                            using (System.Drawing.Image newImage = new Bitmap(oldImage, Width, Height))
                            {
                                using (MemoryStream stream = new MemoryStream())
                                {
                                    ...
                                    Debug.Print(format.ToStringTrim());
        ...
        public static ImageFormat format { get; set; }
        /// <summary>
        /// 将图片按照指定的宽高度截取并输出
        /// </summary>
        ...
        public static MemoryStream ScalingImage<T>(this T model, bool byWidth, System.Web.HttpServerUtilityBase Server, Int32 Width = 100, Int32 Height = 100) where T : class,IModelImageUrl, new()
        {
            ...
            if (image.ImageUrl == null)
            {
                using (MemoryStream stream = new MemoryStream())
                {
                    NetWorkGroup.Extensions.Properties.Resources.NoImage.Save(stream, ImageFormat.Jpeg);
                    return stream;
                }
            }
            else
                        ...
                        using (System.Drawing.Image oldPic = new Bitmap(fileinfo.FullName.Trim()))
                        {
                            ...
                            using (System.Drawing.Image newPic = (byWidth ? new Bitmap(oldPic, Width, p) : new Bitmap(oldPic, p, Height)))
                            {
                                using (MemoryStream stream = new MemoryStream())
                                {
                                    ...
                                    Debug.Print(format.ToStringTrim());
                                    newPic.Save(SavedPath, format);
                                    newPic.Save(stream, format);
                                    return stream;
                                }
                            }
                        }
        ...
        public static MvcHtmlString Pager<T>(this HtmlHelper helper, PagerOptions<T> pagerOptions)
        {
            ...
            if (pagerOptions.MaxRecord > pagerOptions.PageSize)
            {
                ...
                if (page - 1 > 0)
                {
                    sbPager.Append("<a class=\"pre\" href=\"" + currentPageUrl + spliter + "Page=" + (page - 1) + "\">上一页</a>");
                }
                ...
                if (page + 1 <= maxPage)
                {
                    sbPager.Append("<a class=\"next\"" + (page >= maxPage ? " href=\"javascript:;\"  class=\"disabled\"" : " href=\"" + currentPageUrl + spliter + "Page=" + (page + 1) + "\"") + ">下一页</a>");
                }
                sbPager.Append("<a class=\"last\"" + (page >= maxPage ? " href=\"javascript:;\"  class=\"disabled\"" : " href=\"" + currentPageUrl + spliter + "Page=" + maxPage + "\"") + ">末页</a>");
        ...
        public static void IsHave<T>(this IQueryable<T> obj, T theObj, ref Boolean objs, String FixName = "") where T : IModelID<Int64>, IModelParentID<Int64>, IModelName, new()
        {
            if (obj.Count(p => p.ID.Equals(theObj.PID)) > 0)
            {
                foreach (var item in obj.Where(p => p.ID.Equals(theObj.PID)))
                {
                    if (!item.Name.Contains(FixName))
                    {
                        IsHave<T>(obj, item, ref objs, FixName);
                    }
                    else
                    {
                        objs = true;
        ...
        /// 获取指定栏目的所有父栏目
        /// </summary>
        /// <typeparam name="T">实现了IModelID，IModelParentID，IModelName接口的成员</typeparam>
        /// <param name="obj">要查找的元数据</param>
        /// <param name="theObj">指定的目标数据</param>
        /// <param name="objs">ref 结果</param>
        public static void GetParentItem<T>(this IQueryable<T> obj, T theObj, ref List<T> objs) where T : IModelID<Int64>, IModelParentID<Int64>, IModelName, new()
        {
            if (obj.Count(p => p.ID.Equals(theObj.PID)) > 0)
            {
                foreach (var item in obj.Where(p => p.ID.Equals(theObj.PID)))
                {
                    objs.Add(item);
                    GetParentItem<T>(obj, item, ref objs);
                }
            }
        }
        ...
        /// <param name="theObj">指定的目标数据</param>
        /// <param name="objs">ref 结果</param>
        public static void GetParentItemFinall<T>(this IQueryable<T> obj, T theObj, ref List<T> objs) where T : IModelID<Int64>, IModelParentID<Int64>, IModelName, new()
        {
            foreach (var item in obj.Where(p => p.ID.Equals(theObj.PID)))
            {
                if (item.PID == 0 && objs.Count(p => p.Equals(item)) == 0)
                {
                    objs.Add(item);
                }
        ...
        public static IEnumerable<SelectListItem> GetInnerItemWithOptions<T, TType>(this IQueryable<T> obj, T TheParentItem, Expression<Func<T, bool>> KeyValue,int x) where T : IModelID<TType>, IModelParentID<TType>, IModelName, IModelDropDown<TType>, new()
        {
            ...
            foreach (var item in obj.Where(KeyValue).Where(p => p.PID.Equals(TheParentItem.ID)))
            {
                list.Add(new SelectListItem() { Text = item.Name.Trim(), Value = item.ID.ToString() });
                ShowInner<T, TType>(obj, item.ID, list);
            }
            return list;
        }
        #endregion
